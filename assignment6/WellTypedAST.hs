...
typeCheck decls@(ftypes, vtypes) (BinaryExpr bin expr1 expr2)
  | bin `elem` [Plus, Minus, Mult, Div, Mod] = checkbin bin TInt TInt (expr1, expr2) decls
  | bin `elem` [Or, And] = checkbin bin TBool TBool (expr1, expr2) decls
  | bin `elem` [Eq, Lt, Gt, Ne, Le, Ge] = checkbin bin TInt TBool (expr1, expr2) decls
  | otherwise =
    error $
    ...
    then texpr
      ...
      error $
        "Binary operator \"" ++ show bin
          ++ "\" receives wrongly typed arguments "
          ++ show (expr1, expr2)
