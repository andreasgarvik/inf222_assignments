...
import ExtendedBTLAST
  ( Binop (Div, Gt, Le, Minus, Mod, Mult, Plus),
    Expr (BinaryExpr, FunctionCall, IntLiteral, UNot, VarId),
  )
...
exec (Assign name exp) env store = store'
  where
    (env', store') = updvariable name (eval exp env store) (env, store)
...
powerStmt =
  Block
    [ Assign "z" (IntLiteral 1),
      While
        (BinaryExpr Gt (VarId "y") (IntLiteral 0))
        ( Block
            [ While
                (UNot (FunctionCall "odd" [VarId "y"]))
                ( Block
...
powerContext :: (Environment, Store)
powerContext = addvariable "x" (addvariable "y" newcontext)
...
callPower :: GHC.Int.Int16 -> GHC.Int.Int16 -> ValueDomain
callPower x y =
  getvalue
    "z"
    ( env,
      exec
        ...
        env
        ( snd $
            updvariable "x" (VDInt x) $
              updvariable "y" (VDInt y) (env, store)
        )
    )
  where
    (env, store) = addvariable "z" powerContext

-- | Test program for power function
testpower = do
  print $ "2^14 == " ++ show (callPower 2 14)
  print $ "2^15 == " ++ show (callPower 2 15)
  print $ "2^16 == " ++ show (callPower 2 16)
  print $ "10^4 == " ++ show (callPower 10 4)
  print $ "10^5 == " ++ show (callPower 10 5)
  print $ "10^18 == " ++ show (callPower 10 18)
  print $ "2^62 == " ++ show (callPower 2 62)
  print $ "2^63 == " ++ show (callPower 2 63)
  print $ "2^64 == " ++ show (callPower 2 64)
...
eucliddivContext :: (Environment, Store)
eucliddivContext = addvariable "q" $ addvariable "r" $ addvariable "x" (addvariable "y" newcontext)

-- | Call power function with 2 arguments:
-- execute eucliddiv statement (body of function), and return result from variables "q" and "r" variables.
callEucliddiv :: GHC.Int.Int16 -> GHC.Int.Int16 -> (GHC.Int.Int16, GHC.Int.Int16)
callEucliddiv x y = (q, r)
  where
    -- Update input argument values in context
    (env, store) =
      updvariable "x" (VDInt x) $
        updvariable "y" (VDInt y) eucliddivContext
    -- Compute
    store' = exec eucliddivStmt env store
    -- Extract results
    VDInt q = getvalue "q" (env, store')
    VDInt r = getvalue "r" (env, store')

-- | Comparison function calling div and mod from EBTL.
callDivmod :: Integer -> Integer -> (GHC.Int.Int16, GHC.Int.Int16)
callDivmod x y = (d, m)
  where
...
testeucliddiv = do
  print $ callEucliddiv 14 4
  print $ callDivmod 14 4
  --
  print $ callEucliddiv 45 11
  print $ callDivmod 45 11
